...
vulnerabilities = re.compile(r"Summary of Crawling(?: \(\+\/\-\)|)\s+Links Crawled : (\d+)(?: \((?:=|\+\d+|\-\d+)\)|)\s+\nSummary of Vulnerabilities(?: \(\+\/\-\)|)\s+Severity 5 \"Urgent\" : (\d+)(?: \((?:=|\+\d+|\-\d+)\)|)\s+Severity 4 \"Critical\" : (\d+)(?: \((?:=|\+\d+|\-\d+)\)|)\s+Severity 3 \"Serious\" : (\d+)(?: \((?:=|\+\d+|\-\d+)\)|)\s+Severity 2 \"Medium\" : (\d+)(?: \((?:=|\+\d+|\-\d+)\)|)\s+Severity 1 \"Minimal\" : (\d+)(?: \((?:=|\+\d+|\-\d+)\)|)")
...
msg = email.message_from_string(''.join(sys.stdin.readlines()))

for part in msg.walk():
    if part.get_content_maintype() == 'multipart':
        continue
    text = html2text(str(part.get_payload(decode=True),'utf-8'))
